...
const SERVER_URL: string =
  ...
  (import.meta.env.DEV ? 'http://localhost:4200' : window.location.origin);

/** Email login: session + "code is on its way" state, shared with the other Oink games. */
export const authClient = createAuthClient({ storagePrefix: 'insider', serverUrl: SERVER_URL });
...
const KEY = 'insider.identity.v1';

export interface Identity {
  playerId: string;
  name: string;
  /** preferred avatar; the table hands out another if it is taken */
  avatar: number;
}
...
const ROOM_KEY = 'insider.room.v1';
